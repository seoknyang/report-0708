...
void input_participant(char user_name[][8]); //참가자 이름 입력  
void game_control(char name[][8], int condition[], int *left, int user, int *start, int *end); //게임제어 
void cake_display(char name[][8], int condition[], int left, int start, int end); //먹었거나 남아있는 과자 표시 
...
{
	int i, dice_number;
	cake_display(name, condition, *left, *s, *e); //이름,과자상태(0:먹지않음 1:먹음), 남아있는 과자 갯수, s:안먹은 과자 시작위치 e:끝위치 *index 
