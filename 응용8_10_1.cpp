...
double intro_game(char team_name[][7]); //게임설명, 참가자와 제한시간 입력 
void gotoxy(int x, int y); //커서위치제어 
void display_line(int rand); //줄의 움직임 표시 
void display_start(char t_name[][7], int s_w[], int s_l[]); //줄다리기 초기화면 표시 
void make_decision(int r_s, int s_w[], int s_l[], char t_name[][7]); //우승팀 구분 및 출력 
void game_control(int *r_s, clock_t st, double *ps); //게임 제어  
...
int main(void)
{
	...
	do
	{
		...
		make_decision(r_start, score_win, score_loose, team_name); //우승팀 확인  
...
void game_control(int *r_s, clock_t st, double *ps)
{
	...
	rnd=rand()%100;
	if (rnd%2)
		rnd=rnd%4; //rnd가 짝수일 경우 
	else
		rnd=-rnd%4; //rnd가 홀수일 경우 
	if (rnd<0)
	{
