...
void display_rule(void); //당첨사례와 사례금액 출력하는 함수 
void draw_check02(int c, int r); // 사각형 격자 모양 출력 
void reel_series(int r[][3]); //각 릴에 대한 기호의 순서 결정 
void gotoxy(int x, int y); //커서 위치 제어 
int game_progress(int money); //게임진행 상황 출력, 배팅금액 입력 
void display_reel(char rc[][3], int r[][3], int index); //릴 화면에 출력 
void clear_text(void); //화면에 출력된 문장 지우기 
void game_control(char reel[][3], int reel_num[][3], int *money); //게임 제어 
int  return_money(int r[], int betting, int *case_n); //당첨사례에 따라 금액 계산 

int main(void)
{
	char reel[6][3]={"★", "♠", "◆", "♥", "♣", "○"}; //릴에 표시할 기호, 숫자 
	int  i, reel_num[3][3];
	int money=10000; //금액 1만원 초기화 
	srand(time(NULL));  
	printf("  슬롯머신 게임\n\n");
	display_rule();
	gotoxy(3, 5);
