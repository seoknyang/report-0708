...
void game_control(int order, char name[7], char sol[][10], double check[][10], int quest); //게임제어 
void display_result(int user, int order[], double check[][10], char name[][7], char sol[][10], int quest); //결과 표시(시간, 맞은개수) 
...
int main(void)
{
   ...
   for(i=0;i<user_number;i++)
   {
 	rnd=rand()%user_number; //참가자의 게임순서 랜덤 변경  
	temp=order[i]; //*임시저장소(temp) 이용하여 변경  
	order[i]=order[rnd];
	order[rnd]=temp;
   }
   for(i=0;i<user_number;i++)
...
void game_control(int order, char name[7], char sol[][10], double check[][10], int quest)
{
   ...
   for(i=0;i<quest;i++)
   {
	...
	if (rnd[1]>1)
	{
	       rnd[2]=rand()%2;
	       rnd[3]=rand()%2;
	       printf("%d: %s %s %s %s", i+1, flag[rnd[0]], question[rnd[1]], flag[rnd[2]], question[rnd[3]]); //청기백기 문제 출력  
  	}
	else
	printf("%d: %s %s", i+1, flag[rnd[0]], question[rnd[1]]);
    	key=getch();
	end=clock(); 
	check[order][i]=(double)(end-start)/CLK_TCK; //반응시간 계산  
	if (rnd[1]>1)
	{
...
void display_result(int user, int order[], double check[][10], char name[][7], char sol[][10], int quest)
{
   ...
   for(j=0;j<quest;j++)
   {
	...
	for(i=0;i<user;i++)
	{   
		if (sol[order[i]][j]  && check[order[i]][j]<1) //맞았을 경우 반응시간  
		{
			printf("O");
			result[order[i]]+=1;
		}
		else
			printf("X"); //틀렸을 경우 반응시간  
